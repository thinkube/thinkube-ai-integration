...
2. **Scaffold the TEP FIRST — never review in chat.** Chat is a bad reading surface; the file is the review surface. Call `write_tep { tep: {id} }` with no body to lay down the `TEP-TEMPLATE.md` skeleton + canonical frontmatter, then tell the user to open `teps/TEP-{id}.md` with **Markdown Preview to the side** (`Ctrl+K V`). _(Then ask your first question.)_
3. **Interview the user, section by section.** Ask in chat, but land every agreed draft into the FILE — the user reads the rendered preview, chat only steers. `write_tep` replaces the whole body, so each update is **read-modify-write**: `get_thinkube_file teps/TEP-{id}.md` to fetch the current body, apply your change, then `write_tep { tep: {id}, body }` the full body back. Always re-fetch immediately before each `write_tep`; never clobber text you didn't write. Set the title via `write_tep { tep, title }`.
