 ...
 *
 * The vendored React app is unaware of where the thinking space lives — it just asks
 ...
 *   - future adapters (file-based, multi-thinking space, etc.) slot in here without
...
export interface StorageAdapter {
  load(): Promise<ThinkingSpace>;
  save(thinkingSpace: ThinkingSpace): Promise<void>;
  /**
  ...
  /** Set/clear a card's due date (thinking space DATE field). Optional. */
  setDueDate?(id: string, date: string | null): Promise<void>;
  /** Optional event for adapters that observe out-of-band changes. */
  readonly onExternalChange?: vscode.Event<ThinkingSpace>;
  /** Adapter-supplied label used in the panel title. */
  readonly scope: string;
  /** The backing thinking space's identity (root repo, sidecar thinking space dir, display name), so a command
   *  triggered from THIS panel acts on its thinking space rather than the ambient sidebar selection.
