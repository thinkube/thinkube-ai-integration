...
- → Done: verifier green for the slice, and the AC it satisfies is checked on the Spec — **the AI ticks an AC only with concrete evidence (a passing test / command output); it never self-certifies an outcome only a human can confirm (visual UI, live/deploy behaviour). Such outcomes are decomposed into AI-verifiable criteria, or confirmed at the human acceptance gate — never an AI-ticked box on an unrun/unbuilt capability.** (Reviewer + verifier both run in this one gate — no Review/Verify handoff.)
...
**The lever — ceremony = f(risk, nature, AI-testability).** At the start of a piece of work the AI **proposes a level** for each of three inputs — **risk** (consequence × recovery-cost), **nature** (`PoC | production-fix | professional`), and **AI-testability** (can the AI verify it cheaply, alone?) — **surfacing the risk _mechanism_**; the human **overrides** each, and **the AI never blocks on its own assessment** (the override stands; the human owns it). The levels scale ceremony on three axes: **risk floors the _safety_ gates** (verify depth, PR-vs-`main`, reversibility); **nature ceilings the _process_ ceremony** (docs, slicing, acceptance bless, AC formalism); **AI-testability shapes _how_ an AC is made verifiable, never whether it gates** — every Done-gating AC stays **AI-verifiable with concrete evidence**, and the AI ticks it only with that evidence; an outcome only a human can confirm (visual UI, live/deploy behaviour) is **not** an AI-ticked AC — decompose it into verifiable criteria, or confirm it at the human acceptance gate / as a follow-up (no-dodge guard: still automate everything cheaply automatable). _Implemented ≠ tested ≠ self-certified: the AI may never mark Done a capability it has not actually run or built._ **`(low-risk × PoC × high-AI-testability)` is the express lane** — ~zero ceremony (write → smoke-check → ship). No level ever weakens the structural invariants (worktrees, green-before-Done, read-back). _(SP-tgsdvw / TEP-tgs1tf.)_
