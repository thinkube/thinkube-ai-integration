...
- **Low AI-testability raises the bar, it doesn't lower it.** When an outcome is hard to verify directly (visual UI, live/deploy behaviour), do not drop it to a human-executed AC — design AI-verifiable **secondary controls / mitigations** that stand in (a headless render / snapshot check, a data-contract or message-protocol test, an AI probe over positive/negative cases). Low testability means **deeper AC design with more controls**, never fewer checks and never "the human confirms it." (This is the lever's AI-testability axis — it shapes AC depth, never whether an AC is AI-verifiable.)
